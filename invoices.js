...
function openModalPasta() {
  ...
  document.getElementById('modal-pasta-title').textContent = 'Nova Pasta';
  document.getElementById('modal-pasta-save-btn').textContent = 'Criar Pasta';
...
function openEditPasta(idx) {
  const p = pastas[idx];
  if (!p) return;
  selectedPastaCor = PASTA_CORES.findIndex(c => c.bg === p.cor.bg);
  if (selectedPastaCor === -1) selectedPastaCor = 0;

  document.getElementById('p-edit-id').value = idx;
  document.getElementById('modal-pasta-title').textContent = 'Editar Pasta';
  document.getElementById('modal-pasta-save-btn').textContent = 'Guardar Alterações';
  document.getElementById('p-cor-opts').innerHTML = PASTA_CORES.map((c, i) => `
    <div onclick="selectCor(${i})" id="cor-opt-${i}"
      style="width:26px;height:26px;border-radius:50%;background:${c.bg};
             border:2px solid ${i === selectedPastaCor ? c.text : c.border};cursor:pointer"
      title="${c.name}"></div>`).join('');
...
function savePasta() {
  ...
  const editIdxRaw = document.getElementById('p-edit-id').value;
  const icon = document.getElementById('p-icon').value;
  const cor  = PASTA_CORES[selectedPastaCor];

  if (editIdxRaw !== '') {
    const idx = parseInt(editIdxRaw);
    pastas[idx] = { ...pastas[idx], nome, icon, cor };
    save();
    closeModal('modal-pasta');
    renderPastas();
    if (_currentPastaDetailIdx === idx) renderPastaDetail(idx);
    toast('Pasta atualizada!', 'success');
  } else {
    pastas.push({ id: Date.now(), nome, icon, cor });
    save();
    closeModal('modal-pasta');
    renderPastas();
...
// ─── Vista de detalhe da pasta ────────────────────────────────────────────────
let _currentPastaDetailIdx = null;

function openPastaDetail(idx) {
  _currentPastaDetailIdx = idx;
  document.getElementById('pastas-list-view').style.display = 'none';
  document.getElementById('pastas-detail-view').style.display = 'block';
  renderPastaDetail(idx);
}

function closePastaDetail() {
  _currentPastaDetailIdx = null;
  document.getElementById('pastas-detail-view').style.display = 'none';
  document.getElementById('pastas-list-view').style.display = 'block';
  renderPastas();
}

function renderPastaDetail(idx) {
  const p = pastas[idx];
  if (!p) { closePastaDetail(); return; }

  const items = invoices.filter(i => i.pastaId === p.id)
    .sort((a, b) => (b.addedAt || '').localeCompare(a.addedAt || ''));

  document.getElementById('pasta-detail-title').textContent = `${p.icon} ${p.nome}`;
  document.getElementById('pasta-detail-sub').textContent = `${items.length} fatura${items.length !== 1 ? 's' : ''}`;

  const body  = document.getElementById('pasta-detail-body');
  const empty = document.getElementById('pasta-detail-empty');
