...
function renderPastas() {
  ...
  grid.innerHTML = pastas.map((p, idx) => {
    ...
    return `<div class="folder-card" style="border-color:${p.cor.border};background:${p.cor.bg}" onclick="openPastaDetail(${idx})">
      <div style="position:absolute;top:8px;right:8px;display:flex;gap:2px" class="fc-actions">
        <button class="icon-btn" onclick="event.stopPropagation();openEditPasta(${idx})" title="Editar" style="background:rgba(255,255,255,.6)">✎</button>
        <button class="icon-btn" onclick="event.stopPropagation();delPasta(${idx})" title="Apagar" style="background:rgba(255,255,255,.6)">✕</button>
      </div>
